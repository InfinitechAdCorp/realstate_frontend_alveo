          ...
          {images.map((image, index) => (
            <div
              ...
              className={`relative inline-block ml-5 cursor-pointer items-center sm:ml-8 justify-center lg:mt-5  ${
                clickedIndex === index
                  ? 'bg-blue-400 shadow-lg p-3 items-center rounded-xl'
                  : ''
              }`}
            ...
            >
              <div className='card'>
                <img
                  src={
                    building.path &&
                    building.path.startsWith('http://localhost:8000/') // Check if building.path exists and is a full URL
                      ...
                      ? `http://localhost:8000${building.path.replace(
                          /\\/g,
                          '/'
                        )}` // Prepend the base URL for /property/ paths
