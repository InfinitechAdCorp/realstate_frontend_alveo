...
const handleSubmit = async (e) => {
  ...
  payload.append('personalInformation[firstName]', formData.personalInformation.firstName);
  payload.append('personalInformation[lastName]', formData.personalInformation.lastName);
  payload.append('personalInformation[email]', formData.personalInformation.email);
  payload.append('personalInformation[phone]', formData.personalInformation.phone);
  
  payload.append('propertyInformation[name]', formData.propertyInformation.name);
  payload.append('propertyInformation[location]', formData.propertyInformation.location);
  payload.append('propertyInformation[price]', formData.propertyInformation.price);
  payload.append('propertyInformation[status]', formData.propertyInformation.status);
  payload.append('propertyInformation[description]', formData.propertyInformation.description);
  ...
  formData.propertyInformation.files.forEach((fileData, index) => {
    ...
    if (fileData.startsWith('data:image/')) {
      ...
      payload.append('propertyInformation[files][]', blob, `file_${index}.png`);
    } else {
  ...
  <div className="w-1/3 p-8 bg-white shadow-lg rounded-lg mr-6">
    <h2 className="font-semibold text-2xl text-gray-700 mb-6 text-center">Data Privacy Reminder</h2>
    <p className="text-lg text-gray-600 mb-4 indent-10 text-justify leading-10">
      By submitting your property information, you agree that the data provided will be used solely for the purposes of managing property listings and related communications.
    </p>
    <p className="text-lg text-gray-600 mb-4 indent-10 text-justify leading-10">
      Your personal details such as name, contact number, and email will be kept confidential and will not be shared with any third parties without your consent.
    </p>
    <p className="text-lg text-gray-600 mb-4 indent-10 text-justify leading-10">
      Please ensure that the information you provide is accurate and does not contain any sensitive personal details unless absolutely necessary.
    </p>
    <p className="text-lg text-gray-600 mb-4 indent-10 text-justify leading-10">
      You may review our Privacy Policy for more information on how we handle your data.
    </p>
  </div>
...
{/* Render Image Previews */}
 <div className="mt-6">
            {filePreviews.length > 0 && (
              <div>
                <h2 className="text-sm font-medium text-gray-700">Image Previews</h2>
